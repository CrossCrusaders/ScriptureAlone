<template>
    ...
    <div class="flex justify-center pb-8">
        <div class="w-1/3 flex flex-col">
            <RouterLink class="pb-1" v-for="resource of getHalfOne" :to="resource.link || '/'"><Icon icon-name="paper" :size="8"></Icon> {{ resource.title }}</RouterLink>
        </div>
        <div class="pl-4 pr-4 flex flex-row items-center justify-center">
            <div class="h-full w-1 bg-slate-400"></div>
        </div>
        <div class="w-1/3 flex flex-col">
            <RouterLink class="pb-1" v-for="resource of getHalfTwo" :to="resource.link || '/'"><Icon icon-name="paper" :size="8"></Icon> {{ resource.title }}</RouterLink>
        </div>
    </div>
</template>
