<template>
    ...
    <div class="flex justify-center pb-8 pt-4">
        <div class="w-1/3 flex flex-col">
            <RouterLink class="pb-1" v-for="resource of getHalfOne" :to="`/${baseUrl}/${resource.id}` || '/'">
                <Icon icon-name="file-document" :size="24"></Icon> {{ resource.title }}
            </RouterLink>
        </div>
        ...
        <div class="w-1/3 flex flex-col">
            <RouterLink class="pb-1" v-for="resource of getHalfTwo" :to="`/${baseUrl}/${resource.id}` || '/'">
                <Icon icon-name="file-document" :size="24"></Icon> {{ resource.title }}
            </RouterLink>
        </div>
    </div>
