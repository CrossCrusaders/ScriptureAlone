    ...
    <AppLayout>
        <PageContent>
            <h1 id="page-title__sermon-authors"
                class="max-w-prose mx-auto text-slate-800 text-center text-5xl font-title font-bold mt-8 mb-8">
                Search Your Notes
            </h1>
            <div class="max-w-prose mx-auto mb-8">
                <form @submit.prevent="onFormSubmit">
                    <AppInput placeholder="Search For Notes" v-model="currentQuery">
            ...
            <InfiniteScrollContent @scroll:end="onScrollEnd">
                <NotesPreviewGrid @data:loaded="loading = false" :append-content="true" :per-page="16"
                    :page="currentPage" :query="searchedQuery" :query-params="queryParams">
                </NotesPreviewGrid>
            </InfiniteScrollContent>
            <div v-if="reachedEnd">
                <p class="text-center text-xl font-bold mb-4">End of Results</p>
                <p class="text-center text-xl mb-8 underline">
                    <a href="#page-title__note">
                        Back To Top?
                    </a>
                </p>
            </div>
...
import NotesPreviewGrid from '../components/NotesPreviewGrid.vue'
import PocketBaseClient from '../../api/PocketBaseClient'

const loading = ref<boolean>(false)
const reachedEnd = ref<boolean>(false)
const queryParams = ref<any>({ filter: `user="${PocketBaseClient.authStore.model?.id}"` })
