    ...
    <div v-if="showPlayer" class="z-10">
      ...
      <div
        ...
        <div class="flex flex-row justify-between md:justify-start md:gap-8 w-full md:w-auto items-start">
          <div v-if="contentPageLink">
            <RouterLink :to="contentPageLink">
              ...
              <p class="text-white text-md font-bold font-title underline" :title="globalVideoPayload?.title"> {{
              formatDisplayTitle() }}
              </p>
            </RouterLink>
          </div>
          <div v-else>
            ...
            <p class="text-white text-md font-bold font-title" :title="globalVideoPayload?.title"> {{
            formatDisplayTitle() }}
            </p>
          </div>
        ...
        <div class="h-fit md:flex-auto md:max-w-prose w-full p-4 md:p-0">
          <VideoPlayer :state="globalVideoState" :video-src="globalVideoPayload?.url"></VideoPlayer>
        </div>
...
const onCloseClicked = () => {
  setGlobalVideoState(VideoPlayerState.hidden)
}

const contentPageLink = computed(() => {
  return globalVideoPayload.value?.contentPage || ''
})

const formatDisplayTitle = () => {
  return breakpoint.value === 'sm' ?
    formatMaxLengthText(globalVideoPayload.value?.title || '',
      34) : globalVideoPayload.value?.title
}
